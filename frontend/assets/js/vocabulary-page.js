        ...
        <tr class="vocab-table-row hover:bg-surface-container-low/60">
          <td data-label="Word" class="px-6 py-5">
            <div class="flex flex-col">
              <span class="text-lg font-bold text-primary">${escapeHtml(item.lemma)}</span>
              <span class="font-label text-xs uppercase tracking-[0.14em] text-outline">${escapeHtml(item.partOfSpeech)}</span>
            </div>
          </td>
          <td data-label="Meaning" class="px-6 py-5 text-on-surface-variant">${escapeHtml(item.translation)}</td>
          <td data-label="Level" class="px-6 py-5 text-center"><span class="rounded-full bg-surface-container-high px-3 py-1 font-label text-[10px] font-bold uppercase tracking-[0.18em] text-primary">${escapeHtml(item.level)}</span></td>
